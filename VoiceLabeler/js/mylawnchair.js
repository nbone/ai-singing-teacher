 ...
 *
 * Modified March 2014 by Nic Bone (fix some bugs, tailor to my needs)
 */
var Lawnchair = function (options, callback) {
    ...
    if (options.adapters) {
		for (var a = 0; !adapter && a < options.adapters.length; a++) {
			for (var i = 0, l = Lawnchair.adapters.length; !adapter && i < l; i++) {
				if (Lawnchair.adapters[i].adapter === options.adapters[a]) {
					if (Lawnchair.adapters[i].valid()) {
